...
if (empty($_POST['name'])) {
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    try {
      $stmt = $db->prepare("SELECT id FROM users8 WHERE login =?");
    ...
    try {
      $stmt = $db->prepare("INSERT INTO application8 SET name = ?, email = ?, year = ?, gender = ?, kon = ?, bio = ?");//, login = ?, password = ?
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
