...
function show_tables($db){
  $sql = 'SELECT  application8.*, 
                    SuperDef.name as power,
                    users8.login
            FROM application8
            INNER JOIN Superpowers8
                ON application8.id = Superpowers8.id
            INNER JOIN SuperDef 
                ON Superpowers8.superpowers = SuperDef.id
            INNER JOIN users8 
                ON users8.id = application8.id;';
  ?>
  <table class="table">
...
function delete_user($db, $del){
  try{
    $sth = $db->prepare("DELETE FROM application8 WHERE id = ?");
    $sth->execute(array($del));
    $sth = $db->prepare("DELETE FROM Superpowers8 WHERE id = ?");
    $sth->execute(array($del));
    $sth = $db->prepare("DELETE FROM users8 WHERE id = ?");
    $sth->execute(array($del));
  }
...
function add_user($db){
  $sth = $db->prepare("SELECT login FROM users8");
  $sth->execute();
  ...
  try {
    $stmt = $db->prepare("INSERT INTO application8 SET name = ?, email = ?, year = ?, gender = ?, kon = ?, bio = ?");
    $stmt -> execute(array(
        $_POST['name'],
        $_POST['email'],
        $_POST['year'],
    ...
    $id_db = $db->lastInsertId("application8");
    //реализация атомарности
    $stmt = $db->prepare("INSERT INTO Superpowers8 SET id = ?, superpowers = ?");
    ...
    $stmt = $db->prepare("INSERT INTO users8 SET login = ?, pass = ?");
    $stmt -> execute(array(
        $login,
        $hash,
      )
...
function edit_user($db, $edit){
  try {
    $stmt = $db->prepare('SELECT * FROM application8 WHERE id=?');
    ...
    $stmt = $db->prepare("UPDATE application8 SET name = ?, email = ?, year = ?, gender = ?, kon = ?, bio = ? WHERE id =?");
    $stmt -> execute(array(
        $name,
        $email,
        $year,
